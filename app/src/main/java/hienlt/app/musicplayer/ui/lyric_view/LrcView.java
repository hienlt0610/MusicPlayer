package hienlt.app.musicplayer.ui.lyric_view;
...
public class LrcView extends View implements ILrcView {
    ...
    private int mMinSeekLineTextSize = 12;
    private int mMaxSeekLineTextSize = 16;
    private int mLrcFontSize = 17;    // font size of lrc
    private int mLrcHightlightFontSize = 15;    // font size of lrc hightlight
    private int mMinLrcFontSize = 15;
    private int mMaxLrcFontSize = 15;
    private int mPaddingY = 5;        // padding of each row
    private int mSeekLinePaddingX = 5; // Seek line padding x
    private int mDisplayMode = DISPLAY_MODE_NORMAL;
    private LrcViewListener mLrcViewListener;
    private int mWrapWidth = 200;
    ...
    private void initSize() {
        mMinSeekFiredOffset = SystemUtils.convertDpToPx(context.getResources(), mMinSeekFiredOffset);
        mHignlightRow = SystemUtils.convertDpToPx(context.getResources(), mHignlightRow);
        mSeekLineTextSize = SystemUtils.SpTopixels(context, mSeekLineTextSize);
        mMinSeekLineTextSize = SystemUtils.SpTopixels(context, mMinSeekLineTextSize);
        mMaxSeekLineTextSize = SystemUtils.SpTopixels(context, mMaxSeekLineTextSize);
        mLrcFontSize = SystemUtils.SpTopixels(context, mLrcFontSize);
        mLrcHightlightFontSize = SystemUtils.SpTopixels(context, mLrcHightlightFontSize);
        mMinLrcFontSize = SystemUtils.SpTopixels(context, mMinLrcFontSize);
        mMaxLrcFontSize = SystemUtils.SpTopixels(context, mMaxLrcFontSize);
        mPaddingY = SystemUtils.convertDpToPx(context.getResources(), mPaddingY);
        mSeekLinePaddingX = SystemUtils.convertDpToPx(context.getResources(), mSeekLinePaddingX);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mHignlightRow < mLrcRows.size())
            highlightText = mLrcRows.get(mHignlightRow).content;
        else
            highlightText = "";
        mPaint.setColor(mHignlightRowColor);
        mPaint.setTextSize(mLrcFontSize);
...
//        if(calculateWidthFromFontSize(highlightText,mLrcFontSize) < mWrapWidth){
//            canvas.drawText(highlightText, rowX, highlightRowY, mPaint);
//        }else{
//            String[] words = highlightText.split("\\s+");
//            String currentLine = words[0];
//            for(int i = 1; i < words.length; i++) {
//                if(calculateWidthFromFontSize(currentLine + words[i], mLrcHightlightFontSize) < mWrapWidth) {
//                    currentLine += " "+words[i];
//                } else {
//                    canvas.drawText(currentLine, rowX, highlightRowY, mPaint);
//                    highlightRowY += calculateHeightFromFontSize(currentLine,mLrcHightlightFontSize);
//                    currentLine = words[i];
//                }
//            }
...
//
        ...
        if (mDisplayMode == DISPLAY_MODE_SEEK) {
            ...
            canvas.drawText(mLrcRows.get(mHignlightRow).strTime, mSeekLinePaddingX, highlightRowY, mPaint);
        }
    ...
    public StaticLayout drawTextAndBreakLine(final Canvas canvas, final Paint paint,
                                             final float x, final float y, final int maxWidth, final String text) {
        String textToDisplay = (text == null) ? "" : text;
        TextPaint mTextPaint = new TextPaint(paint);
        StaticLayout layout = new StaticLayout(textToDisplay, mTextPaint, maxWidth, Layout.Alignment.ALIGN_OPPOSITE, 1.0f, 0, false);
        canvas.save();
